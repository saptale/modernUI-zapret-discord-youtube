...
**Автор оболочки:** [SapTaLe](https://github.com/saptale) | **Оригинальный проект:** [Flowseal/zapret-discord-youtube](https://github.com/Flowseal/zapret-discord-youtube)

> [!IMPORTANT]
> **ЗАПУСКАЙТЕ ОТ ИМЕНИ АДМИНИСТРАТОРА**, иначе программа не сможет корректно завершать процессы zapret.
> Обновлен до запрета 1.9.8!

## 🏁 БЫСТРЫЙ СТАРТ

Скачайте из релизов (Releases) последнюю версию и выберите, что по вкусу. Либо только UI приложение, где вы укажете путь к вашему zapret, либо zapret + UI приложение. В первом варианте ярлык и .exe файл. Во втором варианте я кладу .exe в корень рядом с .bat файлами.

Исходный код UI приложения хранится в папке UI_Source.


Этот форк включает в себя **удобную графическую оболочку (GUI)** для комфортного управления батниками проекта Zapret. Вам больше не нужно искать нужный файл в огромном списке — всё 
аккуратно рассортировано по категориям, поддерживает добавление в "Избранное" и имеет защиту от двойного запуска.

### 🛡️ Безопасность и принцип работы
Программа представляет собой исключительно визуальный интерфейс. **Она никуда не лезет, не требует установки, не отправляет данные в интернет и не вмешивается в работу системы.** Её единственная задача — читать названия `.bat` файлов в папке, запускать выбранный вами скрипт и корректно завершать (убивать) процессы `winws.exe` и окна `cmd` при нажатии кнопки "Остановить".

### 📦 Что скачивать (Раздел Releases)
Лаунчер полностью автономен и **может использоваться с абсолютно любой версией Zapret**. Во вкладке [Releases](../../releases) (справа) доступно два варианта:
1. **Только UI (Оболочка):** Один файл `.exe`. Идеально, если у вас уже установлен и настроен Zapret. Просто скачайте лаунчер и киньте его в папку к вашим батникам.
2. **Полный пакет (UI + Zapret):** Готовый архив со всеми батниками и лаунчером внутри. Отличный вариант для чистой установки "с нуля".

### ⚙️ Инструкция по использованию
1. Поместите файл лаунчера (`.exe`) в корневую папку Zapret (необязательно, располагайте где удобно, главное укажите путь при запуске проги) (туда, где лежат файлы вроде `general.bat`).
2. **Запустите лаунчер от имени Администратора** (это необходимо, чтобы программа имела права закрывать фоновые процессы обхода).
3. Выберите нужную конфигурацию во вкладках или из списка избранного.
4. Нажмите "Запустить". Для смены конфигурации просто выберите другой батник — программа сама предложит закрыть предыдущий процесс.

### 💻 Для разработчиков
Исходный код графического интерфейса (написан на Python с использованием CustomTkinter) полностью открыт и находится в папке [`UI_Source`](./UI_Source). Я абсолютно не против любых модификаций, переделок или использования моего кода в ваших проектах. Пользуйтесь и улучшайте на здоровье!

---
*(Ниже представлена оригинальная документация проекта Zapret)*
